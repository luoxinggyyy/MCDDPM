...
#         return emb
class ConditionalEmbedding2(nn.Module): #条件编码
...
class ResBlock(nn.Module):  #残差模块
    ...
    def forward(self, x, temb, labels,label2):
        h = self.block1(x)
        h += self.temb_proj(temb)[:, :, None, None, None] #时间
        # labels = labels.to('cuda:0')
        h += self.cond_proj(labels)[:, :, None, None, None] #标签
        h += self.cond_proj(label2)[:, :, None, None, None] #标签
        h = self.block2(h)
...
class UNet(nn.Module): #unet 来进行编码解码
    ...
    def forward(self, x, t, labels,label2):
        ...
        cemb2 = self.cond_embedding2(label2).to('cuda')  #以作测试，放在cpu上的
